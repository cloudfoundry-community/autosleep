...
1) No no-optout without secret
	...
	${result} =                  Run Process  cf  cs  autosleep  default  ${SERVICE_INSTANCE_NAME}  -c  {"inactivity": "${DEFAULT_INACTIVITY}", "excludeAppNameRegExp" : "${EXCLUDE_APP_NAMES}", "no_optout" : "true"}
...
2) No-optout can not unbind
    ...
    ${result} =                  Run Process  cf  cs  autosleep  default  ${SERVICE_INSTANCE_NAME}  -c  {"inactivity": "${DEFAULT_INACTIVITY}", "excludeAppNameRegExp" : "${EXCLUDE_APP_NAMES}", "no_optout" : "true", "secret" : "${DEFAULT_SECRET}"}
