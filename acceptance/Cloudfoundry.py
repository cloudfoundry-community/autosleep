...
class Cloudfoundry(object):
    ...
    def clean_all_service_data(self):
        ...
        instance = self.client.service.get_instance_by_name(self.space_guid, self.instance_name)
        if instance is not None:
            logging.info('clean_all_service_data instance got - %s', instance['metadata']['guid'])
            cleaned = False
            while not cleaned:
                for binding in self.client.service.list_bindings_by_instance(instance['metadata']['guid']):
                    logging.info('clean_all_service_data binding - %s', binding['metadata']['guid'])
                    self.client.service.unbind_application(binding['metadata']['guid'])
                    logging.info('clean_all_service_data - binding deleted')
                try:
                    self.client.service.delete_instance(instance['metadata']['guid'])
                    cleaned = True
                except InvalidStatusCode, ex:
                    if ex.status_code == httplib.BAD_REQUEST and type(ex.body) == dict and ex.body['error_code'] == 'CF-AssociationNotEmpty':
    ...
    def create_service_instance(self, parameters=None):
        if self.instance_guid is not None:
            ...
            instance = self.client.service.create_instance(self.space_guid, self.instance_name, self.plan_guid,
                                                           parameters_sent)
    ...
    def update_service_instance(self, parameters):
        if self.instance_guid is None:
            ...
            self.client.service.update_instance(self.instance_guid, parameters)
    ...
    def delete_service_instance(self):
        if self.instance_guid is None:
            ...
            self.client.service.delete_instance(self.instance_guid)
            self.instance_guid = None
    ...
    def bind_application(self):
        if self.instance_guid is None:
            ...
            binding = self.client.service.bind_application(self.application_guid, self.instance_guid)
    ...
    def unbind_application(self):
        if self.binding_guid is None:
            ...
            self.client.service.unbind_application(self.binding_guid)
            self.binding_guid = None
    ...
    def should_be_bound(self):
        if self.instance_guid is None:
            ...
            binding_guid = self.client.service.get_binding(self.instance_guid, self.application_guid)
            if binding_guid is None:
    ...
    def is_application_bound(self):
        if self.instance_guid is not None:
            ...
            binding_guid = self.client.service.get_binding(self.instance_guid, self.application_guid)
            result = binding_guid is not None
    ...
    def get_bound_applications(self):
        if self.instance_guid is None:
            ...
            result = []
            for app in self.client.service.list_bindings_by_instance(self.instance_guid):
