...
&{DEFAULT_INSTANCE_PARAMETERS}	idle-duration=${DEFAULT_INACTIVITY}	exclude-from-auto-enrollment=${EXCLUDE_ALL_APP_NAMES}

*** Settings ***
Library			Cloudfoundry	${CLIENT_ENDPOINT}    ${CLIENT_SKIP_SSL}	${CLIENT_USER}	${CLIENT_PASSWORD}      ${ORGANIZATION_NAME}    ${SPACE_NAME}	${TESTED_APP_NAME}	${AUTOSLEEP_ENDPOINT}	${SERVICE_NAME}  ${USER_NAME}  ${USER_PASSWORD}	 ${SERVICE_INSTANCE_NAME}  ${DEFAULT_INSTANCE_PARAMETERS}
